...
AC_DEFUN([PR_FUNC_SETGRENT_VOID], [
  AC_CACHE_CHECK(
    [whether setgrent returns void], [ac_cv_func_setgrent_void],
    [AC_RUN_IFELSE([
      #include <sys/types.h>
      #include <grp.h>
  ...
  if test $ac_cv_func_setgrent_void = yes; then
    AC_DEFINE(SETGRENT_VOID, 1, [Define to 1 if the `setgrent' function returns void instead of `int'])
  fi
])
...
AC_DEFUN([PR_CHECK_SS_FAMILY], [
  ...
  AC_COMPILE_IFELSE(
    [AC_LANG_PROGRAM([[
      ...
      AC_MSG_RESULT(yes)
      AC_DEFINE(HAVE_SS_FAMILY, 1, [Define if you have sockaddr_storage.ss_family])
    ], [
      AC_MSG_RESULT(no)
      ...
      AC_COMPILE_IFELSE(
        [AC_LANG_PROGRAM([[
          ...
          AC_MSG_RESULT(yes)
          AC_DEFINE(HAVE___SS_FAMILY, 1, [Define if you have sockaddr_store.__ss_family])
        ], [
          AC_MSG_RESULT(no)
        ])
      ])
...
AC_DEFUN([PR_CHECK_SS_LEN], [
  ...
  AC_COMPILE_IFELSE(
    [AC_LANG_PROGRAM([[
      ...
      AC_MSG_RESULT(yes)
      AC_DEFINE(HAVE_SS_LEN, 1, [Define if you have sockaddr_storage.ss_len])
    ], [
      AC_MSG_RESULT(no)
      ...
      AC_COMPILE_IFELSE(
        [AC_LANG_PROGRAM([[
          ...
          AC_MSG_RESULT(yes)
          AC_DEFINE(HAVE___SS_LEN, 1, [Define if you have sockaddr_storage.__ss_len])
        ], [
          AC_MSG_RESULT(no)
        ])
      ])
