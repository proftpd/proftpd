...
my $TESTS = {
  ...
  site_chmod_limit_denied_ftpaccess => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  site_chmod_failed_pathallowfilter => {
    order => ++$order,
    test_class => [qw(forking)],
  },

  site_chmod_failed_pathdenyfilter => {
    order => ++$order,
    test_class => [qw(forking)],
  },
...
sub site_chmod_limit_denied_ftpaccess {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
...
sub site_chmod_failed_pathallowfilter {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'site');
  ...
  if ($pid) {
    eval {
      ...
      $expected = "CHMOD 444 test.txt: Forbidden filename";
      $self->assert($expected eq $resp_msg,
        test_msg("Expected response message '$expected', got '$resp_msg'"));
...
sub site_chmod_failed_pathdenyfilter {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'site');
