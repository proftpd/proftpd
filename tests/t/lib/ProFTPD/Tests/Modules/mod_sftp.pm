...
my $TESTS = {
  ...
  scp_upload_recursive_dir_bug3447 => {
    order => ++$order,
...
sub sftp_open_trunc_bug3449 {
  ...
  if ($pid) {
    eval {
      ...
      # Bug#3448 occurred because of cached stat values.  To reproduce
      # this, we do:
      #
      # LSTAT test.txt
      # OPEN test.txt
      ...
      # LSTAT test.txt
      #
      # The file size should be different, due to the WRITE. 
...
sub scp_upload_recursive_dir_bug3447 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
