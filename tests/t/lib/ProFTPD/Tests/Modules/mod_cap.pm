...
my $TESTS = {
  ...
  cap_rootrevoke_off_bug3839 => {
    order => ++$order,
    test_class => [qw(bug forking rootprivs)],
  },

  cap_rootrevoke_default_bug3839 => {
    order => ++$order,
    test_class => [qw(bug forking rootprivs)],
  },

  cap_sftp_setreuid_eagain_bug3923 => {
    order => ++$order,
...
sub cap_dac_override {
  ...
  my $uid = 0;
  my $gid = 0;
  my $setup = test_setup($tmpdir, 'cap', undef, undef, undef, $uid, $gid);
...
sub cap_rootrevoke_off_bug3839 {
  ...
  # permissions/privs set for the account we create
  if ($< == 0) {
    unless (chmod(0711, $setup->{home_dir})) {
