...
my $TESTS = {
  ...
  rewrite_map_utf8trans_long_keys => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },

  # TODO: Need tests for RewriteMap using text file
};
...
sub rewrite_map_lowercase {
  ...
  my $test_path = File::Spec->rel2abs("$sub_dir/$test_file");
  ...
  if ($pid) {
    eval {
      ...
      # everything.
      my ($resp_code, $resp_msg) = $client->size(uc("tmp/$test_file"));
      $client->quit();
...
sub rewrite_map_spaces_underscores {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg) = $client->size("tmp/$cmd_file");
      $client->quit();
...
sub rewrite_map_whitespace_trim {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'rewrite');
  ...
  if ($pid) {
    eval {
      ...
      # Send the path with leading spaces; the rewrite rules should handle it
      ...
      # Send the path with leading and following spaces; the rewrite rules
      # should handle it.
      ($resp_code, $resp_msg) = $client->size("    test.txt   ");
      $client->quit();
...
sub rewrite_rule_append_pid {
  ...
  if ($pid) {
    eval {
      ...
      $expected = 'test.txt\.(\d+): No such file or directory';
      $self->assert(qr/$expected/, $resp_msg,
        test_msg("Expected message '$expected', got '$resp_msg'"));
    };
    if ($@) {
      $ex = $@;
    }
...
sub rewrite_bug2915 {
  ...
  if ($< == 0) {
    ...
    unless (chown($setup->{uid}, $setup->{gid}, $sub_dir, $test_path)) {
      die("Can't set owner of $sub_dir to $setup->{uid}/$setup->{gid}: $!");
    }
  }
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg) = $client->site("chmod 600 tmp\\\\$test_file");
      $client->quit();
...
sub rewrite_bug3034 {
  ...
  if ($pid) {
    eval {
      ...
      my ($resp_code, $resp_msg) = $client->port('1,2,3,4,10,11');
...
sub rewrite_rule_env_var_failed {
  ...
  my $test_path = File::Spec->rel2abs("$setup->{home_dir}/$test_file");
  ...
  if ($< == 0) {
    unless (chown($setup->{uid}, $setup->{gid}, $test_path)) {
      die("Can't set owner of $test_path to $setup->{uid}/$setup->{gid}: $!");
    }
  }
...
sub rewrite_rule_env_var_ok {
  ...
  if ($pid) {
    eval {
      ...
      # Send the path with spaces; the rewrite rules should handle it
      my ($resp_code, $resp_msg) = $client->size("foo.txt");
...
sub rewrite_cond_rename_var_bug3029 {
  ...
  if ($< == 0) {
    unless (chmod(0755, $src_dir)) {
      die("Can't set perms on $src_dir: $!");
    }

    unless (chmod($setup->{uid}, $setup->{gid}, $src_dir)) {
      die("Can't set owner of $src_dir to $setup->{uid}/$setup->{gid}: $!");
    }
  }
...
sub rewrite_rule_time_hour_var_bug3673 {
  ...
  my $dt = DateTime->now();
  my $time_zone = 'America/Los_Angeles';
  ...
  if ($ENV{TEST_VERBOSE}) {
    print STDERR "# Creating file $test_path\n";
  }
  ...
  my $config = {
    ...
    # Make sure our ProFTPD processes use the same time zone
    SetEnv => "TZ $time_zone",
...
sub rewrite_rule_time_min_var_bug3673 {
  ...
  my $config = {
    ...
    # Make sure the ProFTPD processes use the same time zone
    SetEnv => "TZ $time_zone",
...
sub rewrite_map_utf8trans_long_keys {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  my $setup = test_setup($tmpdir, 'rewrite');
  ...
  if ($pid) {
    eval {
      ...
      my $long_username = 'A' x 2048;
      eval { $client->login($long_username, $setup->{passwd}) };
      unless ($@) {
