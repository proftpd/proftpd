...
my $TESTS = {
  ...
  sql_passwd_md5_transform_salt_password_bug3500 => {
    order => ++$order,
  ...
  sql_passwd_md5_rounds_tranform_salt_password_bug3500 => {
    order => ++$order,
    test_class => [qw(bug forking)],
  },
...
sub sql_passwd_md5_transform_salt_password_bug3500 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
...
sub sql_passwd_md5_rounds_transform_salt_password_bug3500 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
