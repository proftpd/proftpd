...
my $TESTS = {
  ...
  hidegroup_multiple_groups_bug3530 => {
    order => ++$order,
...
sub hidegroup_multiple_groups_bug3530 {
  my $self = shift;
  my $tmpdir = $self->{tmpdir};
  ...
  auth_group_write($auth_group_file, 'wheel', 0);
  ...
  unless (chown(0, 0, $auth_user_file, $auth_group_file)) {
    die("Can't set owner of files to 0/0: $!");
  }

  unless (chown(1, 1, $config_file)) {
