...
sub subdirs_mkd_denied_pwd_allowed_bug3077 {
  ...
  my $config = {
    ...
    Limit => {
      ...
      # NOTE: if XPWD is not included in this list, then the PWD issued
      # during the session will be denied.  Which sucks; it's fallout
      # from Bug#3077.  I'm not sure how to deal with that case just yet;
      # I think it would require changing the return values of
      # dir_check()/dir_check_full(), and/or adding a new API for this.
      'CDUP CWD PWD LIST NLST RNFR RNTO TYPE XPWD' => {
        AllowAll => '',
      },
    },
  };
