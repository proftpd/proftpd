...
/* usage: FSCacheOptions on|off|size {count} [maxAge {age}] */
MODRET set_fscacheoptions(cmd_rec *cmd) {
  register unsigned int i;
  config_rec *c;
  ...
  for (i = 1; i < cmd->argc; i++) {
    if (strncasecmp(cmd->argv[i], "size", 5) == 0) {
      ...
      *((unsigned int *) c->argv[2]) = max_age;
      ...
      CONF_ERROR(cmd, pstrcat(cmd->tmp_pool, "unknown FSCacheOption: ",
        cmd->argv[i], NULL));
    }
  }
...
static conftable core_conftab[] = {
  ...
  { "From",			add_from,			NULL },
  { "FSCacheOptions",		set_fscacheoptions,		NULL },
