...
static unsigned char *fxp_resp_buffer_alloc(pool *p, size_t sz) {
  unsigned char *buf;

  /* We deliberately allocate 4 bytes more than requested, saving space for
   * for the 4-byte length header/prefix, that will be added later by
   * fxp_packet_write().
   */
  buf = palloc(p, sz + sizeof(uint32_t));

  /* Now we increment the pointer past those 4 bytes, so that the caller is
   * none the wiser.
   */
  buf += sizeof(uint32_t);
...
static int fxp_packet_write(struct fxp_packet *fxp) {
  ...
  /* We know, a priori, that the payload pointer is actually 4 bytes past its
   * actual start.
   */
  buflen = bufsz = fxp->payload_sz + 4;
  buf = ptr = fxp->payload;

  /* ...now take back the 4 bytes for whose length header this buffer is... */
  buf -= sizeof(uint32_t);
  ptr -= sizeof(uint32_t);

  sftp_msg_write_int(&buf, &buflen, fxp->payload_sz);
  return sftp_channel_write_data(fxp->pool, fxp->channel_id, ptr, bufsz);
}
...
static void fxp_version_add_std_ext_names(pool *p, unsigned char **buf,
  ...
  if (fxp_ext_flags & SFTP_FXP_EXT_COPY_DATA) {
    ext_count++;
  }
  ...
  if (fxp_ext_flags & SFTP_FXP_EXT_COPY_DATA) {
    pr_trace_msg(trace_channel, 11, "%s", "+ SFTP extension name: copy-data");
    sftp_msg_write_string(buf, buflen, "copy-data");
  }
...
static int fxp_handle_ext_check_file(struct fxp_packet *fxp, char *digest_list,
  ...
  buf = ptr = fxp_resp_buffer_alloc(fxp->pool, bufsz);
...
static int fxp_handle_ext_copy_data(struct fxp_packet *fxp,
  ...
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, copy_bufsz, status_code;
  uint64_t len;
  struct fxp_packet *resp;
  cmd_rec *cmd, *cmd2;
  struct fxp_handle *src_fxh, *dst_fxh;
  char *copy_buf;
  ...
  cmd = pr_cmd_alloc(fxp->pool, 4, pstrdup(fxp->pool, "SITE"),
    pstrdup(fxp->pool, "COPY"), src_name, dst_name);
  cmd->arg = pstrcat(fxp->pool, "COPY ", src_name, " ", dst_name, NULL);
  cmd->cmd_class = CL_MISC|CL_WRITE;

  src_fxh = fxp_handle_get(src_name);
  if (src_fxh == NULL) {
    pr_trace_msg(trace_channel, 17,
      "copy-data: unable to find handle for name '%s': %s", src_name,
      strerror(errno));
  }

  dst_fxh = fxp_handle_get(dst_name);
  if (dst_fxh == NULL) {
    pr_trace_msg(trace_channel, 17,
      "copy-data: unable to find handle for name '%s': %s", dst_name,
      strerror(errno));
  }
  ...
  src_path = src_fxh->fh->fh_path;
  dst_path = dst_fxh->fh->fh_path;

  if (pr_cmd_dispatch_phase(cmd, PRE_CMD, 0) < 0) {
    status_code = SSH2_FX_PERMISSION_DENIED;

    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "COPY of '%s' to '%s' blocked by '%s' handler: %s", src_path, dst_path,
  ...
  /* Don't allow copying to/from the same handle, same file, or same inode;
   * the pr_fs_copy_file2() function has similar checks.
   */
  if (strcmp(src_name, dst_name) == 0 ||
      strcmp(src_path, dst_path) == 0 ||
      (src_fxh->fh_st->st_dev != 0 &&
       dst_fxh->fh_st->st_dev != 0 &&
       src_fxh->fh_st->st_dev == dst_fxh->fh_st->st_dev &&
       src_fxh->fh_st->st_ino == dst_fxh->fh_st->st_ino)) {
    xerrno = EPERM;

    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "COPY of '%s' to same path '%s', rejecting", src_path, dst_path);
  ...
  res = pr_fsio_fstat(dst_fxh->fh, dst_fxh->fh_st);
  if (res == 0) {
    unsigned char *allow_overwrite = NULL;
    int limit_allow;

    allow_overwrite = get_param_ptr(get_dir_ctxt(fxp->pool, (char *) dst_path),
      "AllowOverwrite", FALSE);

    cmd2 = pr_cmd_alloc(fxp->pool, 3, "SITE_COPY", src_path, dst_path);
    cmd2->arg = pstrdup(fxp->pool, cmd->arg);
    limit_allow = dir_check(fxp->pool, cmd2, "WRITE", dst_path, NULL);
    ...
    if ((allow_overwrite == NULL ||
      ...
      if (!limit_allow) {
        (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
          "COPY to '%s' blocked by <Limit> configuration", dst_path);

      } else {
        (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
          "AllowOverwrite permission denied for '%s'", dst_path);
      }
  ...
  if (fxp_path_pass_regex_filters(fxp->pool, "COPY", src_path) < 0 ||
      fxp_path_pass_regex_filters(fxp->pool, "COPY", dst_path) < 0) {
    xerrno = errno;
  ...
  /* Use a multiple of the block size, for hopefully more efficient I/O. */
  copy_bufsz = (src_fxh->fh_st->st_blksize * 8);
  if (copy_len > 0 &&
      copy_bufsz > copy_len) {
    copy_bufsz = copy_len;
  }
  copy_buf = palloc(fxp->pool, copy_bufsz);

  /* If the client-provided length to copy is zero at this point, use the
   * largest possible value it can be.
   */
  if (copy_len == 0) {
    copy_len = UINT64_MAX - src_offset;
  }

  len = MIN(copy_bufsz, copy_len);
  res = pr_fsio_read(src_fxh->fh, copy_buf, len);
  while (copy_len > 0 &&
         res > 0) {
    size_t datalen;
    off_t offset;
    ...
    while (datalen > 0) {
      res = pr_fsio_write(dst_fxh->fh, copy_buf + offset, datalen);
      if (res < 0) {
        xerrno = errno;
        ...
        (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
          "error copying data to '%s': %s", dst_path, strerror(xerrno));
  ...
  pr_fs_clear_cache2(dst_path);
  pr_fsio_fstat(dst_fxh->fh, dst_fxh->fh_st);
...
static int fxp_handle_ext_copy_file(struct fxp_packet *fxp, char *src,
  ...
  pr_trace_msg(trace_channel, 7, "received extension request: copy-file %s %s",
    src, dst);
...
#if defined(PR_USE_XATTR)
static int xattr_pre_cmd_dispatch(struct fxp_packet *fxp, cmd_rec *cmd) {
  int res;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  struct fxp_packet *resp;
  ...
  /* Whether we wrote our response or not, we need to return an error here,
   * to skip the rest of the handling of this request.
   */
  errno = EPERM;
  return -1;
}

static int xattr_dir_check(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *cmd_group, const char *path, const char *alt_cmd_name) {
  int res;
  const char *cmd_name;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  ...
  res = dir_check(fxp->pool, cmd, cmd_group, path, NULL);
  if (res == 1) {
    cmd_name = cmd->argv[0];
    pr_cmd_set_name(cmd, alt_cmd_name);
  ...
  (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
    "%s %s of '%s' blocked by <Limit> configuration", (char *) cmd->argv[0],
    (char *) cmd->argv[1], path);
...
static int xattr_read_dir_check(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *path) {
  return xattr_dir_check(fxp, cmd, G_READ, path, "STAT");
}

static int xattr_write_dir_check(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *path) {
  return xattr_dir_check(fxp, cmd, G_WRITE, path, "SETSTAT");
}

static int xattr_filters_allow_path(cmd_rec *cmd, const char *path) {
  int res;
  ...
  switch (res) {
    ...
    case PR_FILTER_ERR_FAILS_ALLOW_FILTER:
      pr_trace_msg(trace_channel, 3, "%s %s on '%s' denied by PathAllowFilter",
        (char *) cmd->argv[0], (char *) cmd->argv[1], path);
      errno = EPERM;
      return -1;

    case PR_FILTER_ERR_FAILS_DENY_FILTER:
      pr_trace_msg(trace_channel, 3, "%s %s on '%s' denied by PathDenyFilter",
        (char *) cmd->argv[0], (char *) cmd->argv[1], path);
      errno = EPERM;
      return -1;
  }
...
static int fxp_handle_ext_getxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *path, const char *name, uint32_t valsz) {
  ssize_t res;
  void *val;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  ...
  res = xattr_pre_cmd_dispatch(fxp, cmd);
  if (res < 0) {
    return res;
  }
  ...
  if (xattr_read_dir_check(fxp, cmd, path) < 0) {
    return 0;
  }
...
static int fxp_handle_ext_fgetxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *handle, const char *name, uint32_t valsz) {
  ssize_t res;
  void *val;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  ...
  if (fxh->dirh != NULL) {
     ...
     * by path instead.
     */
    return fxp_handle_ext_getxattr(fxp, cmd, fxh->dir, name, valsz);
  }
...
static int fxp_handle_ext_listxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *path) {
  register unsigned int i;
  int res;
  unsigned char *buf, *ptr;
...
static int fxp_handle_ext_flistxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *handle) {
  register unsigned int i;
  int res;
  unsigned char *buf, *ptr;
  ...
  if (fxh->dirh != NULL) {
     ...
     * by path instead.
     */
    return fxp_handle_ext_listxattr(fxp, cmd, fxh->dir);
  }
...
static int fxp_handle_ext_removexattr(struct fxp_packet *fxp, cmd_rec *cmd,
  ...
  if (xattr_write_dir_check(fxp, cmd, path) < 0) {
    return 0;
  }

  if (xattr_filters_allow_path(cmd, path) < 0) {
    int xerrno = errno;

    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "removexattr(2) on '%s' for attribute '%s' rejected: %s", path, name,
      strerror(xerrno));
...
static int fxp_handle_ext_fremovexattr(struct fxp_packet *fxp, cmd_rec *cmd,
  ...
  if (fxh->dirh != NULL) {
     ...
     * by path instead.
     */
    return fxp_handle_ext_removexattr(fxp, cmd, fxh->dir, name);
  }
  ...
  if (xattr_filters_allow_path(cmd, path) < 0) {
    ...
    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "fremovexattr(2) on '%s' for attribute '%s' rejected: %s", path, name,
      strerror(xerrno));
...
static int fxp_handle_ext_setxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *path, const char *name, void *val, uint32_t valsz,
    uint32_t pflags) {
  int res, flags = 0;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  ...
  if (xattr_filters_allow_path(cmd, path) < 0) {
    ...
    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "setxattr(2) on '%s' for attribute '%s' rejected: %s", path, name,
      strerror(xerrno));
...
static int fxp_handle_ext_fsetxattr(struct fxp_packet *fxp, cmd_rec *cmd,
    const char *handle, const char *name, void *val, uint32_t valsz,
    uint32_t pflags) {
  int res, flags = 0;
  unsigned char *buf, *ptr;
  uint32_t buflen, bufsz, status_code;
  ...
  if (fxh->dirh != NULL) {
     ...
     * by path instead.
     */
    return fxp_handle_ext_setxattr(fxp, cmd, fxh->dir, name, val, valsz,
      pflags);
  }
  ...
  if (xattr_filters_allow_path(cmd, path) < 0) {
    ...
    (void) pr_log_writefile(sftp_logfd, MOD_SFTP_VERSION,
      "fsetxattr(2) on '%s' for attribute '%s' rejected: %s", path, name,
      strerror(xerrno));
...
static int fxp_handle_extended(struct fxp_packet *fxp) {
  ...
  if ((fxp_ext_flags & SFTP_FXP_EXT_COPY_DATA) &&
      strcmp(ext_request_name, "copy-data") == 0) {
    char *src, *dst;
    uint64_t src_offset, dst_offset, copy_len;

    src = sftp_msg_read_string(fxp->pool, &fxp->payload, &fxp->payload_sz);
    src_offset = sftp_msg_read_long(fxp->pool, &fxp->payload, &fxp->payload_sz);
    copy_len = sftp_msg_read_long(fxp->pool, &fxp->payload, &fxp->payload_sz);
  ...
  if (fxp_ext_flags & SFTP_FXP_EXT_XATTR) {
    if (strcmp(ext_request_name, "fgetxattr@proftpd.org") == 0) {
      ...
      res = fxp_handle_ext_fgetxattr(fxp, cmd, handle, name, valsz);
      if (res == 0) {
        fxp_cmd_dispatch(cmd);
    ...
    if (strcmp(ext_request_name, "flistxattr@proftpd.org") == 0) {
      ...
      res = fxp_handle_ext_flistxattr(fxp, cmd, handle);
      if (res == 0) {
        fxp_cmd_dispatch(cmd);
    ...
    if (strcmp(ext_request_name, "fremovexattr@proftpd.org") == 0) {
      ...
      res = fxp_handle_ext_fremovexattr(fxp, cmd, handle, name);
      if (res == 0) {
        fxp_cmd_dispatch(cmd);
    ...
    if (strcmp(ext_request_name, "fsetxattr@proftpd.org") == 0) {
      ...
      res = fxp_handle_ext_fsetxattr(fxp, cmd, handle, name, val, valsz,
        pflags);
      if (res == 0) {
        fxp_cmd_dispatch(cmd);
